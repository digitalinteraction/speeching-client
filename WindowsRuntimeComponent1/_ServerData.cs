...
namespace SpeechingShared
{
    ...
    public static class ServerData
    {
                ...
                foreach(ISpeechingActivityItem act in currentActs)
                {
                    foreach (ActivityCategory cat in AppData.session.categories)
                    {
                        if(Array.IndexOf(cat.activities, act) == -1)
                        {
                            ...
                            if(AppData.IO.DirectoryExists(localResourcesDirectory))
                            {
                                AppData.IO.DeleteDirectory(localResourcesDirectory, true);
                            }
                            break;
                        }
                    }
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    try
                    {
                        ...
                        AppData.IO.WriteToFile(localPath, data);
        ...
        private static async Task<string> UploadFile(string toUrl, string fromUrl, HttpClientHandler credentials)
        {
            using(HttpClient client = new HttpClient(credentials))
            {
                using(StreamContent content = new StreamContent(AppData.IO.OpenFileStream(fromUrl)))
                {
        ...
        public static async Task PushResult(IResultItem toUpload, Action onUpdate, Action<bool> callback, CancellationToken cancelToken)
        {
            ...
            try
            {
                if(toUpload.UploadState < Utils.UploadStage.OnStorage)
                {
                    ...
                    {
                        await UploadFile(storageServer + storageRemoteDir + filename, toUpload.ResourceUrl, handler);
                    }
        ...
        public static async Task<ResultPackage> FetchResultItemWithResources(int resultId)
        {
            ...
            if(AppData.IO.DirectoryExists(extractPath))
            {
                string[] files = AppData.IO.GetFiles(extractPath);
            ...
            try
            {
                ...
                zip = new ZipFile(AppData.IO.OpenFileStream(ret.resultItem.ResourceUrl));
            ...
            finally
            {
                if (zip != null)
                {
                    ...
                    zip.Close();
                    AppData.IO.DeleteFile(ret.resultItem.ResourceUrl);
                }
            }
